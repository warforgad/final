...
def test_reactor():
    ...
    assert r_data['id'] == server.generate_id(info) and r_data['command']['command'] == 'test'
    ...
    assert r.status_code == 200 
    r_data = json.loads(r.text)
    assert r_data['id'] == server.generate_id(info) and r_data['command']['command'] == 'foo'
    ...
    assert r.status_code == 200 
    r_data = json.loads(r.text)
    assert r_data['id'] == server.generate_id(info) and r_data['command']['command'] == 'sleep'
